"""Canonical (spec-faithful) probe eval: sklearn closed-form heads + n_passes=20.

Differs from probe_eval.py (Adam-trained nn.Linear probes) by:
  ...
  - Per-clip predictions saved as flat (n_rec * n_passes,) arrays under `test_*_pred` /
    `test_*_target` keys for downstream bootstrap_canonical.py
  - L1 = single Pearson r / AUC / top-k on the full flat test array (no resampling)

Usage
-----
uv run --group eeg python -m eb_jepa.evaluation.probe_eval_canonical \\
...
def run(
    ...
    wandb_group: str = "probe_eval_canonical",
    # Misc
    ...
    out_json : if set, writes per-seed L1 metrics JSON (informational; bootstrap is the
        canonical source of L1+L2 numbers downstream)
    """
    setup_seed(seed)
    ...
    _overrides = {
        ...
        "data.corrca_filters": corrca_filters,
    ...
    if save_predictions_dir:
        ...
        logger.info("Saved canonical predictions: %s", out_npz)
    ...
    try:
        ...
        if wandb_run_id:
            ...
            run = wandb.init(
                ...
                group=wandb_group,
                name=f"probe_eval_canonical_s{seed}",
                config={
        ...
        flat = {f"probe_eval_canonical/L1/{k}": v for k, v in L1.items()}
        run.log(flat)
        run.finish()
