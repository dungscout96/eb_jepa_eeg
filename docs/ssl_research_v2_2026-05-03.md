...
| **Cell B (matched PD density, latest-ckpt)** | xsub masked latent prediction λ=0.5 | step-level K=2 | kc | no | **lum +0.207 / cont +0.166 / pos +0.158 / narr +0.090** | n/a (no contrastive head) |
...
**Cell B vs Phase D, 5-seed kc+Ridge (matched gradient density, 2026-05-03):**
| metric | Phase D | Cell B | Δ | read |
|---|---|---|---|---|
| luminance | 0.208 | 0.207 | −0.001 | dead-tied |
| contrast | 0.159 | 0.166 | +0.007 | within seed noise |
| position | 0.144 | 0.158 | +0.014 | within seed noise; consistent direction |
| narrative | 0.090 | 0.090 | +0.000 | dead-tied — anti-narrative prediction was wrong |

**Cell B is statistically indistinguishable from Phase D at matched gradient density (~1,100 grad steps).** The cross-subject masked latent prediction objective neither helps nor hurts on the headline protocol. Bootstrap + paired t-test pending on saved per-recording predictions before this enters the memo as a confirmed null. See `docs/evaluation_guide.md` for the standardized protocol; bootstrap is mandatory before any final claim is made.
...
**Reporting protocol**: every numerical claim in this doc and downstream reports must follow `docs/evaluation_guide.md`. Bootstrap (B=2000, recording-level) is mandatory; seed-level paired t-tests are reported when an A-vs-B comparison is in scope.
...
- 2026-05-03 (Cell B 5-seed null + evaluation_guide.md created) — Cell B 5-seed kc+Ridge eval at matched Phase D density (~1,100 grad steps) returned dead-tied numbers vs Phase D on every probe (lum −0.001, cont +0.007, pos +0.014, narr 0.000). Documented null. The anti-narrative structural prediction from cross-subject family theory was *wrong* at this gradient density — Cell B doesn't suppress narrative either. Combined with §A3 forensics (99.5% of training loss is pred_loss), pattern is "at 1.1k steps, the choice of SSL objective barely matters." `docs/evaluation_guide.md` created to standardize the eval protocol going forward (regression + AUC + subject metrics + movie_id; bootstrap B=2000 mandatory; seed t-test on request only). Added Cell B row to §0 table + Cell-B-vs-PhaseD comparison block.
