...
| [`RESULTS_epoch_curve.md`](RESULTS_epoch_curve.md) | four arms re-selected on a held-out probe instead of the validation AUC — 2128 checkpoints. **Two findings.** The 4400-step budget was binding for the from-scratch depth-22 arm above S=1000 (worth 0.012–0.016 r, measured against the same arm at 8800 steps) and cost nothing below S=400. And the optimal epoch tracks cohort size only when training from scratch — epoch 25 at S=10 to 775 at S=1863 — while warm-started it barely moves, so a fixed epoch costs the from-scratch arms 3–4× more at small S. Also retracts the budget-is-binding reading on the *warm* arm, where it was an artifact of the AUC metric. |
