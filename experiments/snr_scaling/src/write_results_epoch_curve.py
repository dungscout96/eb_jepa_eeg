...
def main() -> None:
    ...
    w("")
    w("**This file is the e04 arm. It is not the only scannable arm.** Selection reads")
    w("the val split (R5), so the method works for any arm whose pretraining pool")
    w("excludes R5 -- which is a fact about the pool, not about the arm's name:")
    w("")
    w("| arm | init | depth | pool | scannable |")
    w("|---|---|---|---|---|")
    w("| `e04_reve_scaling` | warm (REVE) | 22 | 1863 | yes — **this file** |")
    w("| `e05_random_scaling` (`_nd`, `_ep800`) | from scratch | 22 | 1863 | yes — not yet run |")
    w("| `e03_scaling` | from scratch | 12 | 1863 | yes — not yet run |")
    w("| `e05_addval` | warm | 22 | 2156 | no, R5 in train |")
    w("| `e05_fromscratch` | from scratch | 22 | 2156 | no, R5 in train |")
    w("")
    w("Note the last two rows: `e05_fromscratch` and `e05_random_scaling` are both")
    w("from-scratch depth-22 arms and land on opposite sides of the line, because they")
    w("differ in pool rather than in initialisation. For the 2156-pool arms the only")
    w("unseen split is R6 -- the reported test split -- so they have no held-out")
    w("selection data at all, the same constraint")
